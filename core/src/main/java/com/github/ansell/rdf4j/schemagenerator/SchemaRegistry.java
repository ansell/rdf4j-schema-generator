...
public abstract class SchemaRegistry<T extends Schema> extends AbstractServiceLoader<IRI, T> {
	...
	public SchemaRegistry(Class<T> serviceClass, ClassLoader classLoader, Function<T, IRI> keyLambda) {
		super(serviceClass, classLoader, keyLambda);
	}

	public SchemaRegistry(final Class<T> serviceClass, final ClassLoader classLoader, final Function<T, IRI> keyLambda,
			final JDefaultDict<IRI, Collection<T>> services) {
